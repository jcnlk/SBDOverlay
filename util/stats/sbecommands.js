...
const getRunCount = (catacombs) => {
  let runCount = 0;
  Object.entries(catacombs.floors).forEach(([key, value]) => runCount += value.times_played);
  Object.entries(catacombs.master_mode_floors).forEach(([key, value]) => runCount += value.times_played);
  return runCount;
};

export const statsSBE = { urlFunc, transformFunc, key: "stats-sbe" };